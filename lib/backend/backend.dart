...
import 'schema/new_user_record.dart';
...
export 'schema/new_user_record.dart';
export 'schema/details_record.dart';
export 'schema/quiz_questions_record.dart';

/// Functions to query NewUserRecords (as a Stream and as a Future).
Future<int> queryNewUserRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      NewUserRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<NewUserRecord>> queryNewUserRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
    ...
    queryCollection(
      NewUserRecord.collection,
      NewUserRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<NewUserRecord>> queryNewUserRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future maybeCreateUser(User user) async {
  final userRecord = NewUserRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await NewUserRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createNewUserRecordData(
    email: user.email ??
  ...
  currentUserDocument = NewUserRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createNewUserRecordData(email: email));
}
